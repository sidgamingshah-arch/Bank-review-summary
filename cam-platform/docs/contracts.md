...
- **Code-value masters** — `segment`, `relationship` (and any other reference list) are governed `codelist` masters (`/api/masters/codelists`, payload `{name, description, entries:[{code,label,active,order}]}`), versioned + maker-checker like every master. Templates carry `segment`/`relationship` as codes, validated against the published lists (enforced only once a list exists). Dropdown sources: `GET /api/masters/published/codelist/{key}` → `{key, entries:[{code,label}]}` (active only); `GET /api/masters/published/sections` → `[{code,name}]` (published section prompts, for the template section dropdown).
- **Per-master Excel** — `GET /api/masters/{type}/xlsx-template` downloads a single-master-type workbook and `POST /api/masters/{type}/xlsx-upload` (multipart) imports it as drafts (per-entry report), for each type separately — alongside the combined `bulk-template`/`bulk-upload`.
- `GET /api/masters/opik/status` (`masters:read`) → `{enabled, backend: opik|local, configured, forced_off, has_credentials, detail, url?, project?, sdk_installed?, reachable?}` — where **section-prompt** content is stored. Opik engages automatically only once its path/credentials are configured; **until then the normal database is used** (`backend: "local"`). When engaged, publishing a section prompt writes its text to the Opik prompt library and stamps the returned commit on the version's `provenance.opik`; resolution reads the text back (fail-open to the master-config snapshot). The key is never returned — only `has_credentials`. Standing/agent rules are unaffected.
